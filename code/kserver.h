...
static int k_server_flush_writes(k_server *server){
  ...
  if(!commands){ rc=-1; oom=1; }
    ...
    if(rc==0){
      ...
      request=head;
      for(i=0;i<count&&request;i++){ request->submitted=1; request=request->write_next; }
    }
    K_FREE(commands);
    if(rc==0){
