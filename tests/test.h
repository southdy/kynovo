...
#include <stdlib.h>
#include <string.h>
/* The 64-bit layer lives here: MSVC 6 has no `long long`, no LL/ULL literals, no %llu and neither
   strtoull nor _strtoui64, while MinGW-w64 accepts all of them - which is why this class of defect only
   ever surfaced on the legacy guest.  Gate rule 9 counts the raw spellings under tests/ and allows them
   nowhere, so test sources must use these names instead. */
#if defined(_MSC_VER)
