...
| 4.2 mem backend inode table and the WAL/snapshot worker threads | fixed, `server: refuse mem:// with worker threads` | refused at open with a reason instead of being accepted silently; the manual threaded stress tool is disk-backed now; deterministic test, and disabling the check fails it |
