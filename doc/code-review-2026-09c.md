...
| 3.3 / 3.4 / 3.5 membership visibility truthfulness | fixed, `membership: report only the changes this node submitted` | per-entry source replaces the server-global; a refused change rolls pending back to its pre-submit count; the three auto-replace submit points count a rejection in the backoff; note text fits its 128-byte buffer. Test guard proven to fail (35/36) |
| 4.2 mem backend inode table and the WAL/snapshot worker threads | **fixed: the mem backend is spinlock-protected and the refusal is gone** | exposure verified as the shared bucket head/refcount only, the two over-claims stay retracted in doc/gaps-audit.md §R; the lock covers it and tests/vfs_fault_test.c hammers two same-bucket paths from two threads (5/5 with the lock; with the lock stubbed out the same test reports expected 20000 / actual 124 opens) |
| 6.1 / 6.3 / 6.4 gate holes | fixed, `gates: the // rule can see past an apostrophe, and an empty file list is not a pass` | `scan()` no longer falls back from an empty list to the whole library; a new rule fails on an empty git-derived list; the sprintf budget and its message come from one measured constant (10, not the hard-coded 13). Both holes injected and shown red; rules 18 -> 19 |
| 5.x client / CLI | fixed (four), one item open | piped stdin never ran a command and never exited (rc=124, banner only) because cli.h's input loop is gated on a console; the one-shot budget (3s) was shorter than the client's own request timeout and then claimed "not executed"; PIPE counted a REDIRECT as the operation's outcome, so a fully successful run could exit non-zero; avg_us divided the sum over EVERY operation by the CAPPED sample count; `RGET limit abc` silently became unbounded. Open: a request queued before the connection is established can be dropped without a resend. cli_smoke 12 -> 19 checks, all passing (doc/gaps-audit.md §S) |
| 7 documentation drift | fixed, `doc: the crash contract cites the real write points, by name` | all four crash-contract references were wrong and are re-verified with their enclosing function names; testing.md's retired L0 rule was already corrected in the earlier counts commit |
