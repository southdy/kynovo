...
| 4.2 mem backend inode table and the WAL/snapshot worker threads | refusal kept, **reason rewritten after two of its claims were refuted** | verified exposure is only the shared bucket head / refcount (vfs.h:308 vs 324, ~1 in 1024 bucket collision, needs simultaneity); the "threads would corrupt each other's writes" and "same-inode close/unlink" claims are retracted in doc/gaps-audit.md §R. Locking the backend (deletes the refusal) is the open option |
