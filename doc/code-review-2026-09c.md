...
| 5.x client / CLI | fixed (five) | piped stdin never ran a command and never exited (rc=124, banner only) because cli.h's input loop is gated on a console; the one-shot budget (3s) was shorter than the client's own request timeout and then claimed "not executed"; PIPE counted a REDIRECT as the operation's outcome, so a fully successful run could exit non-zero; avg_us divided the sum over EVERY operation by the CAPPED sample count; `RGET limit abc` silently became unbounded. The item this row used to leave open - a request queued before the connection is established - is **refuted** by a new deterministic client test (kclient_test 18/18): the client does flush it once connected. cli_smoke 12 -> 19 checks, all passing; doc/gaps-audit.md §S and §T |
