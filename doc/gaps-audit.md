...
| area | findings | state |
|------|----------|-------|
| Recovery | **A1 CRITICAL - FIXED (`kserver.h` scan + `test_wal_recovery_refuses_a_prefix_past_the_scan_ceiling`)**; the rest open: A2 (silent empty restart at the 64-segment ceiling, metadata present) `[me]`; A2 ack-slot lags 63 records `[me]`; A3 holes after records read are a silent chain end `[me]`; A4 no generation continuity after a torn tail `[audit]`; A5 base 0 accepted as verified `[me]`; A6 InstallSnapshot writes in place without truncate/end-probe `[me]`; A7 forced-high base decodes the wrong record, verify helper creates files, dead `skipped_prefix` `[audit]` | **open** |
