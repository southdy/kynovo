...
| area | findings | state |
|------|----------|-------|
| Recovery | **A1 CRITICAL - FIXED (`kserver.h` scan + `test_wal_recovery_refuses_a_prefix_past_the_scan_ceiling`)**; **A2/A3 REFUTED (reachable forms) - the misleading slot comment is rewritten and the property pinned by `test_wal_recovery_refuses_a_middle_segment_cut`**; open: A4 (silent empty restart at the 64-segment ceiling, metadata present) `[me]`; A2 ack-slot lags 63 records `[me]`; A3 holes after records read are a silent chain end `[me]`; A4 no generation continuity after a torn tail `[audit]`; A5 base 0 accepted as verified `[me]`; A6 InstallSnapshot writes in place without truncate/end-probe `[me]`; A7 forced-high base decodes the wrong record, verify helper creates files, dead `skipped_prefix` `[audit]` | **open** |
| Threads/lifetime | B1 `k_wal_files_close` "idempotent" only sequentially, called after a 30 s warn-only wait `[me]`; B2 `thread_destroy` frees a runtime whose thread did not join `[audit]`; B3 the event loop writes an inbound snapshot unlocked while the worker may write the same path `[audit]`; B4 rx buffer freed inline while the reader is inside it `[me]`; B5 a failed snapshot-result post wedges the stop path (OOM) `[audit]`; B6 mem open/unlink still read the table through `be` `[me]` | **open** |
| Protocol/membership | C1 the membership note is still server-global (misdelivered/lost) `[me]`; C2 the note's worst case exceeds its 128-byte buffer `[me]`; C3 a refusal resets the whole wait clock `[audit]`; C4 `age_ms` is one global accumulator printed per entry `[audit]`; C5 started/completed counters measure different things `[audit]`; C6 TOPOLOGY exempt from admission but served via a barrier ⇒ redirect ring `[audit]`; C7 a write queued behind a failed FCALL is stranded `[audit]`; C8 phantom pending source, SHUTDOWN tied to its ack `[audit]` | **open** |
| Client/CLI | D1 script mode still hangs on an unreachable host and always exits 0 `[me]`; D2 one-shot reports success for a redirected request and for a failed CAS `[me]`; D3 PIPE percentiles are 5th/9th/9.9th `[me]`; D4 PIPE exits 0 with dropped requests, rate numerator counts redirects `[me]`; D5 deadlines armed at queue time `[audit]`; D6 give-up paths drop requests and can leave `discovering` set `[audit]`; D7 6 s budget on local refusals, argv re-join splits args, connect never retried `[audit]`; D8 EXIT reports failure, no redirect bound, limit guard dead on 32-bit `[audit]` | **open** |
| Tests/gates | E1 the gate has no per-layer timeout `[me]`; E2 a tautological assert in `cemon_test` `[audit]`; E3 the new git-list rule reports before the tests list is built `[me]`; E4 `is_char_literal` cannot lex escapes, two `//`-rule false negatives `[audit]`; E5 `watch_counters.sh` greps names the server never emits and several harnesses advertise gates they do not enforce `[audit]`; E6 `unique` workload flag unreachable (`atoi("unique")`) `[audit]`; E7 assorted rule/needle blind spots `[audit]` | **open** |
