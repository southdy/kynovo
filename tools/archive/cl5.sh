...
for i in 1 2 3; do ./build/kdbsvr.exe init "disk://$ROOT/build/cl5/n$i" >/dev/null || echo "init $i FAILED"; done
SPEC="1@127.0.0.1:8121:8221,2@127.0.0.1:8122:8222,3@127.0.0.1:8123:8223"
start_node(){ ./build/kdbsvr.exe server $1 $2 $3 "disk://$ROOT/build/cl5/n$1" "$SPEC" > build/cl5/s$1.log 2>&1 & }
